...
const rentals = [
    { id: 1, date: "2026-02-01", amount: 150.00, type: "rental", category: "sedan", description: "Toyota Corolla" },
];

const router = new Router();

router.get("/rentals", async (ctx) => {
    ctx.body = rentals;
    ctx.status = 200;
});

router.get("/rental/:id", async (ctx) => {
    const id = parseInt(ctx.params.id);
    const item = rentals.find((t) => t.id === id);
    if (item) {
        ctx.body = item;
        ctx.status = 200;
    } else {
...
router.del("/rental/:id", async (ctx) => {
    const id = parseInt(ctx.params.id);
    const idx = rentals.findIndex((t) => t.id === id);
    if (idx === -1) {
        ctx.throw(404, "Rental not found");
    }
    const deleted = rentals.splice(idx, 1)[0];
    ctx.body = deleted;
    ctx.status = 200;
});

router.get("/allRentals", async (ctx) => {
    ctx.body = rentals;
    ctx.status = 200;
});
